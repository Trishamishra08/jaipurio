...
/** Seeds the Country collection (used by the "Available countries" picker) once, on first boot. */
async function ensureCountries() {
  const count = await Country.countDocuments();
  if (count > 0) return;
  await Country.insertMany(
    COUNTRIES.map(([name, code], index) => ({ name, code, sortOrder: index, isActive: true, status: 'Published' }))
  );
  console.log(`Seeded ${COUNTRIES.length} countries.`);
}

module.exports = ensureCountries;
