...
const mapRow = (r) => ({
  ...
  groupsLabel: Array.isArray(r.groups)
    ? r.groups
        .map((g) => g.groupName || g.name || g.group?.name || '')
        .filter(Boolean)
        .join(', ') || `${r.groups.length} group(s)`
    : '—',
  createdAt: r.createdAt ? String(r.createdAt).slice(0, 10) : '',
});

export const AdminEcommerceSpecificationTables = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        header: 'Groups',
        accessor: 'groupsLabel',
        cell: (row) => <span className="text-slate-600">{row.groupsLabel}</span>,
      },
      {
        header: 'Status',
        accessor: 'status',
  ...
  return (
    <EcommerceLayout breadcrumb={['SPECIFICATION TABLES']}>
      {error ? (
      ...
      {loading ? (
        <div className="py-16 text-center text-sm text-slate-500">Loading tables…</div>
      ) : (
        <AdminDataTable
          columns={columns}
          data={rows}
...
export default AdminEcommerceSpecificationTables;
